...
class LibriSpeechDataset(torch.utils.data.Dataset):
    def __init__(self, subsets, length, stochastic=True, cache=True):
        ...
        cached_id_to_filepath_location = '/raw/librispeech/LibriSpeech__datasetid_to_filepath__subsets={}__length={}.json'.format(
            subsets, length)
        cached_id_to_filepath_location = PATH + cached_id_to_filepath_location

        cached_id_to_sex_location = '/raw/librispeech/LibriSpeech__datasetid_to_sex__subsets={}__length={}.json'.format(
            subsets, length)
        ...
        df = pd.read_csv(PATH + '/raw/LibriSpeech/SPEAKERS.TXT', skiprows=11, delimiter='|', error_bad_lines=False)
        ...
        for s in subsets:
            ...
            subset_len = 0
            for root, folders, files in os.walk(PATH + '/raw/LibriSpeech/{}/'.format(s)):
            ...
            progress_bar.close()
        print('Finished indexing raw. {} usable files found.'.format(self.n_files))
