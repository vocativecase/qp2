...
class Data():
    def __init__(self, simple=True):
        ...
        with open(PATH + '/raw/voc/'+'speakers_to_intervals.json', 'w') as fp:
    ...
    def get_intervals(self):
        for speaker in self.all_speakers:
            transcript_path = PATH + '/raw/voc/box_transcripts/' + speaker + '.txt'
    ...
    def list_of_speakers(self):
        for root, folders, files in os.walk(PATH + '/raw/voc/'):
            for f in files:
                if f.endswith('.wav'):
                    self.audios.append(f.replace('.wav', ''))

        for root, folders, files in os.walk(PATH + '/raw/voc/box_transcripts/'):
            for f in files:
                if f.endswith('.txt'):
                    with open(PATH + '/raw/voc/box_transcripts/' + f, 'r') as text:
                        ids = []
                        for line in text:
                            print(line)
                            print(f)
                        ...
                        if len(ids) == 2 and f.replace('.txt', '') not in self.audios:
                            self.missing_audios.append(f.replace('.txt', ''))
                        if not self.simple:
